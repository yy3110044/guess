...
CREATE TABLE `new_guess_versus` (
  ...
  `allPayBonus` decimal(15, 6) COMMENT '此竞猜发放的总奖金',
  `startTime` datetime COMMENT '开始时间',
  `endTime` datetime COMMENT '结束时间，关闭下注下记录',
  `betPause` bit(1) COMMENT '是否暂停下注',
  `resultItemId` int(11) COMMENT '胜的那个结果new_guess_versus_item id，零表示未结束(还没有结果)，负数表示流局',
  `resultItemName` varchar(256) COMMENT '胜的那个结果new_guess_versus_item name',
  `status` enum('未开始', '进行中', '已结束', '流局') COMMENT '状态',
  `superVersusId` int(11) COMMENT '属于哪个父对阵，零表示没有父对阵',
  `childVersusCount` int(11) COMMENT '子对阵数量',
  `createTime` datetime COMMENT '创建时间',
  `leftTeamLogoUrl` varchar(512) COMMENT '左队伍图标',
  `rightTeamLogoUrl` varchar(512) COMMENT '右队伍图标',
  `leftTeamScore` int(11) COMMENT '左队伍比分',
  `rightTeamScore` int(11) COMMENT '右队伍比分',
  `showTeamLogo` bit(1) COMMENT '显示teamLogo',
  `showTeamScore` bit(1) COMMENT '显示teamScore',
  PRIMARY KEY (`id`),
  KEY `itemId` (`itemId`),
...
CREATE TABLE `new_guess_versus_item` (
  ...
  `betAmount` decimal(15, 6) COMMENT '下注金额',
  `allPayBonus` decimal(15, 6) COMMENT '此竞猜发放的总奖金',
  `useFixedOdds` bit(1) COMMENT '是否使用固定赔率',
  `fixedOdds` decimal(15, 6) COMMENT '固定赔率，当赔率为固定时，使用此值',
  `changeOdds` decimal(15, 6) COMMENT '变动赔率，当赔率为变动时，使用此值',
  `changeOddsPlusRatio` decimal(15, 6) COMMENT '变动赔率增加额度百分比',
  `changeOddsPlusValue` decimal(15, 6) COMMENT '变动赔率增加额度固定值',
  `changeOddsPlusStrategy` tinyint COMMENT '变动赔率增加策略，小于零按百分比，大于零按固定值，零不限制',
  `changeOddsMinusRatio` decimal(15, 6) COMMENT '变动赔率减少额度百分比',
  `changeOddsMinusValue` decimal(15, 6) COMMENT '变动赔率减少额度固定值',
  `changeOddsMinusStrategy` tinyint COMMENT '变动赔率减少策略，小于零按百分比，大于零按固定值，零不限制',
  `changeOddsMin` decimal(15, 6) COMMENT '变动赔率下限，为零表示不限制',
  `changeOddsMax` decimal(15, 6) COMMENT '变动赔率上限，为零表示不限制',
  `createTime` datetime COMMENT '创建时间',
  PRIMARY KEY (`id`),
